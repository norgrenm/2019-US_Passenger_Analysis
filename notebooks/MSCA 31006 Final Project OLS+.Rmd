...
```{r}
ps_ts = ts(as.numeric(gsub(",", "", passenger[,3])), start=c(2002, 10), end=c(2020, 4), frequency=12)
v_ff = ts(as.numeric(gsub(",", "", FedData[,2])), start=c(2002, 10), end=c(2020, 4), frequency=12)
v_un = ts(as.numeric(gsub(",", "", FedData[,3])), start=c(2002, 10), end=c(2020, 4), frequency=12)
v_ps = ts(as.numeric(gsub(",", "", FedData[,4])), start=c(2002, 10), end=c(2020, 4), frequency=12)
...
first_diff_ps_ts_train <- diff(ps_ts_train, lag = 12)##

plot(first_diff_ps_ts_train)## 

##The plot of the first difference of ps_ts_train appear to be non-stationary. 

kpss.test(first_diff_ps_ts_train)## 
...
adf.test(first_diff_ps_ts_train)##

##A Augmented Dickey-Fuller test also indicates the series in 1st difference is no stationary

sec_diff_ps_ts_train <- diff(first_diff_ps_ts_train, lag = 1)##

plot(sec_diff_ps_ts_train)##

kpss.test(sec_diff_ps_ts_train)## 
